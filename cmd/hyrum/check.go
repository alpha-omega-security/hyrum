...
var testRunners = map[string]func(dir string) []string{
	"npm":    func(dir string) []string { return []string{"node", "--test", dir} },
	"pypi":   func(dir string) []string { return []string{"python3", "-m", "pytest", "-q", dir} },
	"golang": func(dir string) []string { return []string{"go", "test", "./" + dir + "/..."} },
}
...
func runTests(ctx context.Context, targetRoot, testDir, ecosystem string) (ok bool, output string, err error) {
	...
	build, ok := testRunners[ecosystem]
	if !ok {
...
func detectManager(dir string) (managers.Manager, error) { //nolint:ireturn // registry lookup
